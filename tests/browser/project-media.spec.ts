	...
	test.describe(device, () => {
		...
		test("focus preview preserves mixed ratios, thumbnail/keyboard navigation and modal focus", async ({
			...
			for (const [key, position] of [
				...
				await expect(thumbs.nth(position)).toHaveAttribute(
					"aria-current",
					"true",
				);
			...
			for (const [index, ratio] of [1.6, 0.5, 3].entries()) {
				...
				expect((box?.width ?? 0) / (box?.height ?? 1)).toBeCloseTo(ratio, 2);
				expect(box?.height).toBeLessThan(page.viewportSize()?.height ?? 0);
				expect(box?.width).toBeLessThan(page.viewportSize()?.width ?? 0);
