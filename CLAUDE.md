...
- Commissioned native SVG identities use optional `brandKit` metadata, `artwork/brands/<id>/v<version>/` masters and immutable `public/brands/<id>/v<version>/` exports. Preserve real Hexly tokens, licensed font outlines, light/dark marks, usage rules and checksummed manifests. Keep source adoption and the product release separate from brand publication; a Hexly-only handoff may have null source/profile revisions. See [Snail's native vector handoff](docs/18-snail-brand.md). Image-generation checkpoints apply to generated images, not these original vector assets.
