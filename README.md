...
首版收录 66 个项目，备份 43 份原始图像；其余 23 个项目沿用已有 Emoji，并明确标注来源。所有项目都有中英文介绍、颜色依据和独立档案。Frogie 已采用第一版动物家族 Logo，旧图和完整创作过程保留在档案中，Pew 是下一位。

## 功能

- **项目导航** — 按分类浏览、搜索中英文名称与描述，访问已核实的站点或源码仓库。
- **Logo 画廊** — 宽幅新旧对照，图标、透明和白底视图，128 / 64 / 32 / 16 px 尺寸与侧栏、浏览器场景；浅深底色检查、可复制色板、完整提示词和原始文件下载。
...
项目资料统一维护在 [`src/data/projects.json`](src/data/projects.json)。更新 GitHub profile 时，同时更新本站的数据、Logo 备份和色板，再生成预览与档案。普通构建直接使用仓库内的资源，不依赖相邻项目或运行时 GitHub 请求。
