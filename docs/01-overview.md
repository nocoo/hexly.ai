...
The dedicated `skills` category (Skills / 技能) is available for newly onboarded Agent Skill repositories. Existing Skills & MCP entries retain their `extensions` membership until an explicit migration is requested. A Skill's product category is independent of its Logo series; purpose-related 3D objects use `family.series: "material"`.
...
The directory uses `/`, its logo wall `/logos`, project details `/projects/<id>`, and reusable compositions `/templates`. Category, search and sort remain query parameters. Template composition choices also remain shareable. Brand anchors and video selections use hashes with native smooth scrolling and a reduced-motion fallback. The shared alias helper preserves former page URLs without redirecting asset paths. Direct loads and browser history resolve the same project and filters. Archived projects remain accessible through their own paths.
