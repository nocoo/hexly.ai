...
The site chrome follows the connected Play / Journal / Résumé / Portfolio frame. The sticky header keeps the hexly.ai mark, highlights Portfolio among those four destinations, and uses the shared language and theme icon controls. Projects and Logo gallery sit beside the mark as local views, without a second full-width rule. The footer is the two-band family footer: the shared `zheng li.` wordmark, English copyright, version, the four destinations, the Beijing signature, `WEB · MOBILE · AI`, and back to top. Markdown and `llms.txt` links are omitted because this site does not publish those documents.
