...
The directory uses compact horizontal cards with a prominent logo, title, description, and a labeled GitHub repository link: three columns on large screens, two on tablets, and one on phones. The whole card opens its logo detail; the independent GitHub link opens the repository. Card links support ordinary browser navigation and opening in another tab. Logos are centered beside a stable text block; the `Refined` / `已重绘` badge, category, and miniature palette share a full-width footer. Redraw status does not create another set of sections.
