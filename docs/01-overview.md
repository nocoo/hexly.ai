...
The site chrome follows the connected Play / Journal / Résumé / Portfolio frame. The sticky header keeps the hexly.ai mark, highlights Portfolio among those four destinations, and uses the shared language and theme icon controls. Projects and Logo gallery form a compact segmented switch with small icons and a raised active state. Above 1000 px, the switch sits beside the brand in a single header row. All smaller widths share two rows: brand and preferences above, the four destinations on the left and the view switch on the right below. The switch keeps its labels above 560 px and uses icons at narrower widths, with full accessible names throughout. Header gutters come directly from the same shell as the page content; grid alignment controls spacing, and the lower row has 12 px of padding before the divider. The header offset includes that padding for in-page navigation. The desktop footer has two bands: the shared `zheng li.` wordmark, English copyright, version, the four destinations, `llms.txt`, the Beijing signature, `WEB · MOBILE · AI`, and back to top. At 640 px and below, it becomes one compact band with the wordmark, short English copyright, and a back-to-top button; secondary metadata and repeated destinations are hidden.
