...
On 2026-09-08 the owner requested that the site omit its own project. The current directory contains 69 projects (49 active and 20 archived); the GitHub profile is unchanged. The site's own identity is preserved separately in `src/data/site-identity.json` and its historical profile. Default display order is animals, templates, games, then tools, using the dated popularity snapshot in `src/data/project-order.json`. Existing numbered profile filenames remain stable across membership and display-order changes.
