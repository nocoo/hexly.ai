...
The September 10 Showtime preparation brings the catalogue to 71 projects (51 active and 20 archived). Showtime now leads the final tools group before InfoSpace and is first in the curated Tools filter through `featuredTools`; A–Z remains alphabetical. Its source is an explicitly recorded local snapshot while the initial repository commit and README work are pending. See [Showtime onboarding](13-showtime-onboarding.md).
