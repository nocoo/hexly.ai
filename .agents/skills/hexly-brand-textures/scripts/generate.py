...
def generate_one(row, theme, attempt):
    ...
    prompt = ((run / "prompt.txt") if (run / "prompt.txt").is_file()
              else (study / theme / "prompt.txt")).read_text()
    ...
    def record_open(request, **kwargs):
        ...
        try:
            with native_open(request, **kwargs) as response:
                ...
                limits = {key: value for key, value in response.headers.items()
            ...
            save(run / "response.json", {
                ...
                "rateLimits": {key: value for key, value in error.headers.items()
                               if key.lower().startswith("x-ratelimit-")},
...
def main():
    ...
    parser.add_argument("--attempt", type=int, choices=range(1, 5), default=1)
    parser.add_argument("--jobs", type=int, choices=range(1, 5), default=1)
    parser.add_argument("--interval", type=float, default=35, help="Minimum seconds between request starts")
    ...
    rows = [r for r in data["projects"] if r["action"] == "generate" and not r.get("archived", False)
            and not json.loads(Path(f"src/data/projects/{r['id']}.json").read_text()).get("archived", False)
            and (not args.project or r["id"] in args.project)]
    if args.project and set(args.project) != {r["id"] for r in rows}:
        parser.error("Unknown or excluded project; archived entries receive basic support only")
    ...
    if args.failed_only:
        if args.attempt < 2:
            parser.error("--failed-only requires an explicit later attempt")
        tasks = [(row, theme) for row, theme in tasks
                 if not any(json.loads(path.read_text())["status"] == "succeeded"
                            and (not path.with_name("raw-review.json").exists()
                                 or json.loads(path.with_name("raw-review.json").read_text())["status"] != "rejected")
                            for path in Path(row["study"]).glob(f"{theme}*/response.json"))]
