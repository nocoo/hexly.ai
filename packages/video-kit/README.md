...
The public manifest contains only component metadata, supported themes and client-preview mode. There are no current sample movies, old posters, exported deck download links or orphan files in `public/video-assets`. Screenshots, contact sheets and representative deck proofs stay outside deployed assets. The five current layouts have no legacy aliases or hidden old implementations. Original `v0.6.0` sample binaries remain recoverable from the verified external Git bundle described in [the history-reduction plan](../../docs/20-r2-assets-execution.md); rewritten tags retain code history, while binary recovery uses the R2 inventory or that original-history backup.
