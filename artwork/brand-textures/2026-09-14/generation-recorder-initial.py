...
def generate_one(row, theme, attempt):
    ...
    def record_open(request, **kwargs):
        ...
        try:
            ...
            save(run / "response.json", {
                ...
                "note": "Error body and authentication headers intentionally omitted. No image received; provider billing outcome is not asserted.",
            })
            raise
    ...
    try:
        ...
        return 3 if receipt.get("httpStatus") in (401, 403) else 2
    finally:
...
def main():
    ...
    if not args.generate:
        print(json.dumps({"model": "gpt-image-2.5-flare", "projects": [r["id"] for r in rows],
                          "themes": themes, "attempt": args.attempt, "maximumRequests": len(rows)*len(themes)}, indent=2))
        return
    ...
    with ThreadPoolExecutor(max_workers=args.jobs) as pool:
        futures = [pool.submit(task, row, theme) for row in rows for theme in themes]
        for future in as_completed(futures):
            codes.append(future.result())
    print(json.dumps({"requestsScheduled": len(codes), "completedOrPreserved": codes.count(0),
                      "uncompleted": sum(code != 0 for code in codes), "authorizationStop": stopped.is_set()}))
