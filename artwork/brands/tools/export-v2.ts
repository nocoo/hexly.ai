// Maintained single-project kit exporter, derived from the frozen 2026-09 collection.
// bun artwork/brands/tools/export-v2.ts <project-id> <brand-version>
...
const toolRoot = "artwork/brands/tools";
...
if (
	...
	throw new Error(
		"Usage: bun artwork/brands/tools/export-v2.ts <project-id> <brand-version>",
	);
const project = readProjects().find((p) => p.id === id && !p.archived);
if (!project) throw new Error(`Missing active catalogue project: ${id}`);
...
for (const project of projects) {
	...
	const source = `artwork/brands/${project.id}/v${version}`;
	const root = `/brands/${project.id}/v${version}`;
	...
	if (
		recipe.project !== project.id ||
		recipe.brandVersion !== version ||
		!/^[a-f0-9]{40}$/.test(recipe.siteBaseline)
	)
		throw new Error(`Recipe identity or baseline mismatch: ${source}`);
	const study = `artwork/logo-family/${family.root.split("/").slice(3, -1).join("/")}`;
	if (recipe.sourceArchive !== study)
		throw new Error(
			`Recipe must reference the current family archive: ${study}`,
		);
	const sourceAdoption =
		recipe.sourceAdoptionRevision ??
		(family.status === "adopted" &&
		project.logo.sha256 === family.foreground.sha256
			? project.source.repositoryRevision
			: null);
	const adoptionNote = sourceAdoption
		? `Source Logo adoption is recorded at ${sourceAdoption}. Brand version ${version} is independent of the product package version; kit assembly changes no runtime or product UI.`
		: "No source adoption is claimed by this kit; existing source identity remains authoritative.";
	...
	const guidelines: NonNullable<Project["brandKit"]>["guidelines"] = [
		...
		},
		{
			...
			description: {
				en: `Navigation 24px preferred, 16px minimum. Wordmark ≥${minWordmark}px wide; lockup ≥${minLockup}px. Use transparent PNG/ICO at small sizes without a tile, extra red dot, shadow or rounded mask. Fine material detail softens at 16px.`,
				zh: `导航推荐 24px，最小 16px。字标宽度至少 ${minWordmark}px，组合至少 ${minLockup}px。小尺寸使用透明 PNG/ICO，不加底板、红点、阴影或圆角遮罩；16px 时细小材质细节会柔化。`,
			},
		},
	];
	await save(
	...
	for (const theme of ["light", "dark"] as const) {
		...
		await save(
			...
			theme === "light"
				...
				: "Night presentation of the exact native foreground; independent Hexly theme surface, no artwork recoloring",
		);
		await save(
			`icon-${theme}-512.png`,
	...
	const tokens = {
		...
		productUI: {
			action: "unchanged",
			source: `Source repository ${project.source.repositoryRevision}; no product UI tokens are prescribed by this kit`,
		},
	};
	await save(
		"tokens.json",
	...
	for (const name of [
		...
		const recordName = `source-${name.replaceAll("/", "-")}`;
		await save(
			recordName,
			bytes,
			"Exact source record; not reformatted or reconstructed",
		);
		records.push({
			path,
			sha256: hash(bytes),
			bytes: bytes.length,
			export: `${root}/${recordName}`,
			url: `https://h.no.mt${root}/${recordName}`,
		});
	}
	let generation = null;
	if (!family.method) {
		...
		generation = {
			...
			madeInThisRelease: recipe.identityGeneratedForThisRelease === true,
			newGenerationCallsDuringKitAssembly: 0,
		};
		await save(
			"prompt.txt",
			prompt,
			"Exact approved identity generation prompt; kit assembly makes no new image-model call",
		);
	}
	const sourceRights =
	...
	const provenance = {
		...
		finishing: {
			...
			sourceAdoptionRevision: sourceAdoption,
			firstIndependentIdentity: family.previous === null,
		},
		build: {
			baseline: recipe.siteBaseline,
			recipe: `${source}/recipe.json`,
			recipeSha256: hash(await readFile(`${source}/recipe.json`)),
			tool: `${toolRoot}/export-v2.ts`,
			toolSha256: hash(await readFile(`${toolRoot}/export-v2.ts`)),
			typographyTool: `${toolRoot}/outline.py`,
			typographyToolSha256: hash(await readFile(`${toolRoot}/outline.py`)),
			sharp: "0.35.4",
		},
	};
	await save(
	...
	const standalone = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="robots" content="noindex,follow"><meta name="color-scheme" content="light dark"><title>${xml(project.title)} — Hexly brand specimens ${version}</title><link rel="canonical" href="https://hexly.ai/projects/${project.id}"><link rel="icon" href="./favicon.ico"><link rel="stylesheet" href="./review.css"></head><body data-theme="light"><header class="masthead"><a class="family" href="/projects/${project.id}#brand"><img src="./hexly-mark.svg" alt="" width="30" height="34"><span>hexly.ai</span></a><button id="theme-switch" type="button" aria-pressed="false">Dark canvas</button></header><main>
<div class="introduction"><p class="eyebrow">${xml(project.title)} / Hexly campaign archive / ${version}</p><h1>${xml(recipe.caption.en)}</h1><p>${xml(recipe.description.en)}</p><p class="caption">Hexly campaign surfaces only. Original Logo colors and independent product palettes remain unchanged.</p></div>
...
<section aria-labelledby="identity-title"><div class="section-heading"><div><p class="eyebrow">01 / Identity</p><h2 id="identity-title">${family.previous ? "An identity, with its history." : "The first independent identity."}</h2></div><fieldset class="view-switch" aria-label="Artwork presentation"><button type="button" data-view="icon" aria-pressed="true">Icon</button><button type="button" data-view="white" aria-pressed="false">White</button><button type="button" data-view="transparent" aria-pressed="false">Transparent</button></fieldset></div><div class="comparison"${family.previous ? "" : ' style="grid-template-columns:minmax(0,1fr);max-width:720px;margin-inline:auto"'}>${family.previous ? `<figure><a class="artwork previous" href="./${originalName}" download><img src="./${originalName}" width="${originalPixels.width}" height="${originalPixels.height}" alt="${xml(project.title)} recorded original project identity"></a><figcaption><strong>Project identity / Original bytes</strong><span>Original colors, geometry and source record preserved.</span></figcaption></figure>` : ""}<figure><a class="artwork" id="candidate-link" data-view="icon" href="./icon-light.png" download><img id="candidate" src="./icon-light-512.png" width="512" height="512" alt="${xml(project.title)} existing Hexly campaign artwork"></a><figcaption><strong>${family.previous ? "Hexly family / Campaign artwork" : "First identity / Approved project artwork"}</strong><span>${campaign.sameBytesAsOfficial ? "Same foreground as the recorded project identity." : "Separate family interpretation; source adoption is not claimed."}</span></figcaption></figure></div></section>
...
<section aria-labelledby="download-title"><div class="section-heading"><div><p class="eyebrow">08 / Sources & downloads</p><h2 id="download-title">An archive you can trace.</h2></div><p>Version ${version} / official and campaign roles are separate.</p></div><div class="downloads">${[
		...
		.join(
			"",
		)}</div><p class="caption">${xml(sourceRights)}</p>${generation ? `<details><summary>Read the exact historical generation prompt</summary><pre>${xml(await readFile(`${study}/prompt.txt`, "utf8"))}</pre><a href="./prompt.txt" download>Exact prompt ↓</a><p>Model ${xml(generation.model)} · batch ${xml(generation.batch)} · identity origin is recorded; kit assembly made no additional model call.</p></details>` : ""}<p><a href="${family.archive}">Preserved source archive ↗</a> · <a href="./space-grotesk-ofl.txt">Space Grotesk / OFL</a> · <a href="./geist-mono-ofl.txt">Geist Mono / OFL</a></p></section></main><footer><a href="/projects/${project.id}#brand">Back to ${xml(project.title)} ↗</a><span>Hexly / Campaign archive ${version}</span></footer><script src="./review.js"></script></body></html>`;
	await save(
		"review.html",
