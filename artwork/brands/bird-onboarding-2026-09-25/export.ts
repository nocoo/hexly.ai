...
// bun artwork/brands/bird-onboarding-2026-09-25/export.ts [project-id ...]
...
const collection = "artwork/brands/bird-onboarding-2026-09-25";
const inventory = JSON.parse(
	await readFile(
		"artwork/brands/bird-onboarding-2026-09-25/identity-inventory.json",
		"utf8",
	),
);
...
for (const project of projects) {
	...
	for (const theme of ["light", "dark"] as const) {
		...
		for (const square of [false, true]) {
			...
			const leftEntry = recipe.hero.entryEdges.includes("left");
			const artLeft = entry
				? leftEntry
					? 0
					: width - artSize
			...
			if (!square) {
				...
				layers.push({
					input: svg(
						width,
						height,
						`<g transform="translate(${leftEntry ? 1168 : 0} 0)"><circle cx="136" cy="270" r="9" fill="${colors.accent}"/><path d="M172 270h324M136 690h968" fill="none" stroke="${colors.line}" stroke-width="1.5"/></g>`,
					),
				});
			}
	...
	if (!family.method) {
		...
		await save(
			"prompt.txt",
			prompt,
			"Exact approved Logo generation prompt; kit assembly made no image-model call",
		);
	}
	const sourceRights =
...
<section aria-labelledby="download-title"><div class="section-heading"><div><p class="eyebrow">08 / Sources & downloads</p><h2 id="download-title">An archive you can trace.</h2></div><p>Version 1.0.0 / official and campaign roles are separate.</p></div><div class="downloads">${[
		...
		.join(
			"",
		)}</div><p class="caption">${xml(sourceRights)}</p>${generation ? `<details><summary>Read the exact historical generation prompt</summary><pre>${xml(await readFile(`${study}/prompt.txt`, "utf8"))}</pre><a href="./prompt.txt" download>Exact prompt ↓</a><p>Model ${xml(generation.model)} · batch ${xml(generation.batch)} · no new generation during kit assembly.</p></details>` : ""}<p><a href="https://github.com/nocoo/hexly.ai/tree/${inventory.baseline}/${study}">Preserved source archive ↗</a> · <a href="./space-grotesk-ofl.txt">Space Grotesk / OFL</a> · <a href="./geist-mono-ofl.txt">Geist Mono / OFL</a></p></section></main><footer><a href="/projects/${project.id}#brand">Back to ${xml(project.title)} ↗</a><span>Hexly / Campaign archive 1.0.0</span></footer><script src="./review.js"></script></body></html>`;
	await save(
		"review.html",
	...
	project.brandKit = {
		...
		sourceAdoptionRevision: project.source.repositoryRevision,
