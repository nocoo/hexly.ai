...
async function placeLayer(bytes, width, height, framing = {}) {
	...
	const anchor = framing.anchor ?? [0.5, 0.5];
	if (
		anchor.length !== 2 ||
		anchor.some((value) => !Number.isFinite(value) || value < 0 || value > 1)
	)
		throw new Error(
			"Placement anchor must contain two fractions between 0 and 1.",
		);
	const left = Math.floor((width - placedWidth) * anchor[0]);
	const top = Math.floor((height - placedHeight) * anchor[1]);
	const resized = await sharp(bytes)
		.resize(placedWidth, placedHeight)
		.png()
		.toBuffer();
	const extend = framing.extendEdges ?? [];
	if (extend.length) {
		if (extend.some((edge) => !["left", "right", "bottom"].includes(edge)))
			throw new Error(
				"Only documented lower neck/shoulder edges may continue.",
			);
		const source = await sharp(resized).ensureAlpha().raw().toBuffer();
		const output = Buffer.alloc(width * height * 4);
		for (let y = 0; y < height; y++) {
			const sy = y - top;
			if (sy < 0 || (sy >= placedHeight && !extend.includes("bottom")))
				continue;
			for (let x = 0; x < width; x++) {
				const sx = x - left;
				if (
					(sx < 0 && !extend.includes("left")) ||
					(sx >= placedWidth && !extend.includes("right"))
				)
					continue;
				const index =
					(Math.min(sy, placedHeight - 1) * placedWidth +
						Math.max(0, Math.min(sx, placedWidth - 1))) *
					4;
				output.set(source.subarray(index, index + 4), (y * width + x) * 4);
			}
		}
