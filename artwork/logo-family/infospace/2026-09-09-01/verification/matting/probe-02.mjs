...
const foreground = await sharp(result.rgba, {
	...
	.toBuffer();
await writeFile(
	path.join(output, "probe-02-settings.json"),
	`${JSON.stringify(settings, null, "\t")}\n`,
);
await writeFile(path.join(output, "probe-02-protection.svg"), protectionSvg);
await writeFile(path.join(output, "probe-02-transparent.png"), foreground);
for (const [name, background] of [
	...
	await sharp(foreground)
		...
		.resize(1024)
		.png()
		.toFile(path.join(output, `probe-02-${name}.png`));
}
await sharp(foreground)
	...
	.resize(840, 640)
	.png()
	.toFile(path.join(output, "probe-02-lower-edge.png"));
const samples = [
	[1690, 1550],
	[1700, 1550],
	[1500, 1690],
