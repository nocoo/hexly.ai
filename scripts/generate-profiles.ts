...
for (const project of projects) {
	...
	const firstGeneratedIdentity =
		project.family?.previous === null &&
		project.family.status === "adopted" &&
		!project.family.method;
...
${kit.texture?.model ? `- Decorative surface: \`${kit.texture.model}\`, approved native PNG with full-canvas WebP delivery; display once without tiling or cropping. Exact theme prompts and generation receipts are in the archive.\n` : ""}${kit.previousVersion ? `- [Previous v${kit.previousVersion} ${collected ? "archive" : "identity"}](${sourceLink(`/brands/${project.id}/v${kit.previousVersion}/review.html`)}) remains immutable.\n` : ""}
...
${family.previous ? `- [Previous original](${sourceLink(family.previous.original)}), copied from [${family.previous.sourceUrl.startsWith("/") ? "its preserved local source" : "its immutable source"}](${sourceLink(family.previous.sourceUrl)})\n- Previous SHA-256: \`${family.previous.sha256}\`` : "- First project identity; no earlier independent Logo existed at intake."}
